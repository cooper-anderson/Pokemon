...
public class DreamEater extends Move {
	...
	public DreamEater(String name, Type type, AttackType attackType, int power, int accuracy, int points) {
		super(name, type, attackType, power, accuracy, points);
	}

	@Override
	public MoveInstance behavior(Pokemon pokemon) {
		return pokemon.game.instantiate(new DreamEaterInstance(pokemon, this));
	}

	public class DreamEaterInstance extends MoveInstance {
		public DreamEaterInstance(Pokemon pokemon, Move move) {
		...
		public int onCollision(Pokemon pokemon, Projectile projectile) {
			...
			if (pokemon.status == Status.SLEEPING) {
