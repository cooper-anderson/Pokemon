...
public class Cut extends Move {
	public Cut(String name, Type type, AttackType attackType, int power, int accuracy, int points) {
		super(name, type, attackType, power, accuracy, points);
	}

	@Override
	public MoveInstance behavior(Pokemon pokemon) {
		return pokemon.game.instantiate(new CutInstance(pokemon, this));
	}

	public class CutInstance extends MoveInstance {
		private ArrayList<Projectile> projectileList = new ArrayList<>();
		private double angle;
		private int count = 10;

		public CutInstance(Pokemon pokemon, Move move) {
			super(pokemon, move, false, false, true);
			this.angle = this.pokemon.facing.getAngle() - Math.PI / 3;
			this.lifetime = 25;
			for (int i = 0; i < this.count; i++) this.projectileList.add(this.spawnProjectile(new CutProjectile(this, this.move, this.angle, i, count)));
		}
	...
	public class CutProjectile extends Projectile {
		private double angle;
		private int index;
		private int count;

		public CutProjectile(MoveInstance owner, Move move, double angle, int index, int count) {
			super(owner, move);
			this.angle = angle;
			this.index = index;
			this.count = count;
		...
		public void behavior() {
			...
			double dist = 100 * ((double) this.index / (double) this.count);
